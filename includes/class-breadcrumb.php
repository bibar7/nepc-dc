...
class Holycross_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'holycross_get_breadcrumb', $this->crumbs, $this );
	}
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'holycross' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'post' != get_post_type( $post ) ) {
			...
			$ar_title = $this->get_custom_pt_archive_title($post_type, get_post_type( $post ));
			$this->add_crumb( $ar_title, get_post_type_archive_link( get_post_type( $post ) ) );
		} else {
			$cat = '';
			$post_options = get_post_meta( $post->ID, 'holycross_post_options', true);
	...
	private function add_crumbs_post_type_archive() {
		...
		if ( $post_type ) {
			$title = $this->get_custom_pt_archive_title($post_type, get_post_type());
			$this->add_crumb( $title, get_post_type_archive_link( get_post_type() ) );
		}
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( esc_html__( 'Posts tagged', 'holycross' ).sprintf( ' &ldquo;%s&rdquo;', single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( esc_html__( 'Author', 'holycross' ).': '.$userdata->display_name );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( esc_html__( 'Search results for', 'holycross' ).sprintf( ' &ldquo;%s&rdquo;' , get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( esc_html__( 'Page', 'holycross' ).' '.get_query_var( 'paged' ) );
		}
	}
	private function get_custom_pt_archive_title( $ar_post_type, $post_type ){
		if( empty($ar_post_type) ) return '';
		$title = '';
		if ( defined( 'SLZ' ) ) {
			// Array to map extensions with template setting custom title
			$archive_map = slz()->theme->get_config('custom_ac_posttype_template');
		...
		if( empty($title)) {
			$title = $ar_post_type->labels->singular_name;
		}
		return $title;
	}
}