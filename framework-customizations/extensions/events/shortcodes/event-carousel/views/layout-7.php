...
$model->attributes['show_thumbnail'] = isset( $data['image_display'] ) && $data['image_display'] == 'show' ? 'yes' : 'no' ;

// Image size
$cls_container = '';
$model->attributes['thumb-size']['small'] = 'full';
if( isset($model->attributes['is_container']) && $model->attributes['is_container'] ){
    $cls_container = 'container';
    $model->attributes['thumb-size'] = '';
}

$cls_padding = '';
if( isset($model->attributes['has_top_padding']) && $model->attributes['has_top_padding'] ){
...
// If show thumbnail then show block-image-wrapper or hide
$html_block_image = '';
if( $has_image != '' ) {
    $block_image = '';
    ...
    $html_block_image = '<div class="block-image-wrapper">
                             '.$block_image.'%3$s
                             <div class="block-content-wrapper"><div class="'. $cls_container .'">
                                 <div class="block-content">%4$s<ul class="block-info">%5$s%6$s%7$s</ul><div class="block-description">%8$s</div></div>
                             </div></div>
                         </div>';
}

// Html format for render
$html_format = '<div class="main-layout item %1$s"><div class="slz-block-item-07 %13$s style-02">
                    '. $html_block_image .'
                    <div class="block-donation-wrapper"><div class="'. $cls_container .'">
                        <div class="donate-block">
                            <div class="col-left"><div class="remaining-block">
                                <span class="price">%9$s</span><span class="text">'. esc_html__('Remaining to helps', 'holycross') .'</span>
...
<div class="slz-carousel-wrapper">
	<div class="carousel-overflow">
		<div class="slz-carousel slz-events-carousel layout-7 <?php echo esc_attr( $cls_padding ); ?>" data-slidestoshow="<?php echo esc_attr( $data['slide_to_show'] ); ?>" data-autoplay="<?php echo esc_attr( $data['slide_autoplay'] ); ?>" data-isdot="<?php echo esc_attr( $data['slide_dots'] ); ?>" data-isarrow="<?php echo esc_attr( $data['slide_arrows'] ); ?>" data-infinite="<?php echo esc_attr( $data['slide_infinite'] ); ?>" data-speed="<?php echo esc_attr( $data['slide_speed'] ); ?>">
            <?php
            // Loop for get posts
