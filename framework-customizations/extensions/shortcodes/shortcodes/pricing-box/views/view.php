...
$column_class = $out = $css = $custom_css = $has_shadow = '';
		...
		$out .= '<div class="item">';
			$out .= '<div class="slz-pricing-table-01 pricing-box-total-'.esc_attr( $i ).' '.esc_attr( $active ).'">';
				...
				$out .= '<div class="pricing-header">';
					if ( !empty( $data['title'.$i] ) || !empty( $data['sub_title'.$i] ) ) {
						$out .= '<div class="title pricing-box-title-'.esc_attr( $i ).'">';
						if ( !empty( $data['title'.$i] ) ){
							$out .= esc_html( $data['title'.$i] );
						}
...
if ( is_plugin_active( 'js_composer/js_composer.php' ) ) {
	echo '<div class="slz_shortcode slz-pricing-plan-01 '.esc_attr( $block_cls ).''.esc_attr ( $has_shadow ).'">';
		echo '<div class="row">';
			echo ( $out );
		echo '</div>';
	echo '</div>';
