...
if ( !empty( $data['icon_box_3'] ) ) {
	$items = (array) vc_param_group_parse_atts( $data['icon_box_3'] );

	if ( !empty( $items ) ) {
		
		$out .= '<div class="slz-list-block slz-list-column style-03 '. esc_attr( $class_column ) .'">';
		foreach ($items as $item) {
			...
			$out .= '<div class="item">';
				...
				$out .= '<div class="slz-icon-box-1 icon-box-item-'.esc_attr($i).' '.esc_attr( $class_view ).' '. esc_attr( $align ) .'">';
					...
					if(!empty($number)){
						$out .= '<div class="icon-cell">' . wp_kses_post($number) . '</div>';
					}
